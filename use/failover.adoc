...
[role="lead"]
Em caso de desastre, faça failover do seu site principal da VMware no local para outro site local ou VMware Cloud on AWS.

Durante o failover, a cópia Snapshot mais recente do SnapMirror é usada. Ou, você pode selecionar uma cópia Snapshot específica em uma cópia Snapshot pontual (de acordo com a política de retenção do SnapMirror). A opção pontual pode ser útil se você estiver enfrentando um evento de corrupção, como ransomware, onde as réplicas mais recentes já estão comprometidas ou criptografadas. A recuperação de desastres do BlueXP  mostra todos os pontos disponíveis no tempo.

Esse procedimento quebra a relação de replicação, coloca as VMs de origem do vCenter offline e permite a leitura/gravação no site de destino.

Você pode incluir scripts personalizados nos formatos .sh, .bat ou .ps1 como processos de pós-failover. Com scripts personalizados, você pode fazer com que a recuperação de desastres do BlueXP  execute seu script após um processo de failover. Por exemplo, você pode usar um script personalizado para retomar todas as transações de banco de dados após a conclusão do failover.

Antes de iniciar o failover, você pode testar o processo, garantindo o sucesso quando necessário. O teste não coloca as máquinas virtuais offline.



== Teste o processo de failover

Antes de iniciar um failover real, você deve testar o processo de failover.

Durante um teste de failover, as máquinas virtuais são criadas temporariamente. A recuperação de desastres do BlueXP  não mapeia o volume de destino. Em vez disso, ele cria um novo volume FlexClone a partir do instantâneo selecionado e um datastore temporário que faz backup do volume FlexClone é mapeado para os hosts ESXi.
